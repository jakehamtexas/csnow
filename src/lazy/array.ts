...
// export class CollectedArray<T> implements ICollectedArray<T> {
// 	constructor(private readonly array: T[]) {}
// 	collect(): ICollectedArray<T> {
// 		return new CollectedArray(this.array);
// 	}
// 	collectDeep(): DeepCollected<T, Type.Array, never> {
// 		return this.collect().map((item) => (hasType.lazy(item) ? item.collectDeep() : item)) as DeepCollected<T, Type.Array>;
// 	}
// 	iterators: { values(): IterableIterator<T>; indices(): IterableIterator<number>; entries(): IterableIterator<[number, T]> } = (() => {
...
// 		return new CollectedArray(this.array.map(f));
// 	}
// 	each(f: EachFn<T, number>): void {
// 		this.array.forEach(f);
// 	}
// 	filter(f: FilterFn<T, number>): ICollectedArray<T> {
// 		return new CollectedArray(this.array.filter(f));
// 	}
// 	flatten(): FlattenedArray<T, 0> {
// 		return new CollectedArray(this.array.flat()) as never as FlattenedArray<T, 0>;
// 	}
// 	flatMap<U>(f: FlatMapFn<T, number, U, 0>): ICollectedArray<U> {
// 		return new CollectedArray(this.array.flatMap(f).flatMap(eagerlyEvaluate));
// 	}
// 	concat(iterable: SeedIterable<T, number>): ICollectedArray<T> {
// 		return new CollectedArray(this.array.concat(eagerlyEvaluate<T, 0>(iterable)));
// 	}
...
// 		return new CollectedArray(this.array.reduce(f, startingValue));
// 	}
// 	values(): T[] {
...
// class ZipIterator<T> extends LazyArray<ILazyArray<T>> {
// 	constructor(private readonly parent: LazyArray<ILazyArray<T>>) {
// 		super(parent);
// 	}

// 	*[Symbol.iterator]() {
// 		const iterators = Array.from(this.parent.map((arr) => arr.iterators.values()));

// 		if (!iterators.length) return;
// 		forever: while (true) {
// 			const buffer = [];
// 			for (const iterator of iterators.map((iterator) => iterator.next())) {
// 				if (iterator.done) {
// 					break forever;
// 				}

// 				buffer.push(iterator.value);
// 			}
// 			yield new LazyArray(buffer);
// 		}
// 	}
// }
